...
const sourcePdfs: Record<WeekId, { src: string; pages: number }> = {
  1: { src: "https://files.manuscdn.com/user_upload_by_module/session_file/91985099/fIIQNDntvLQmtgZP.pdf", pages: 10 },
  2: { src: "https://files.manuscdn.com/user_upload_by_module/session_file/91985099/zyaiXNkyAWhIaloe.pdf", pages: 12 },
  3: { src: "https://files.manuscdn.com/user_upload_by_module/session_file/91985099/cHVfaBkQgrwelaMO.pdf", pages: 8 },
  4: { src: "https://files.manuscdn.com/user_upload_by_module/session_file/91985099/xzlBklJizUhIAIXq.pdf", pages: 10 },
};
...
function SourcePDF({ week }: { week: WeekId }) {
  const source = sourcePdfs[week];
  return <section className="source-pdf"><div className="source-pdf-heading"><div><SectionLabel>ALL ORIGINAL PAGES / WEEK 0{week}</SectionLabel><h3>첨부 원문 전체 보기</h3><p>이 주차의 {source.pages}쪽 원문 슬라이드를 그대로 확인할 수 있습니다. 웹 요약과 함께 원문을 대조하며 학습하세요.</p></div><a href={source.src} target="_blank" rel="noreferrer"><ScrollText size={16} /> 원문 새 창으로 열기</a></div><details><summary>원문 {source.pages}쪽 펼쳐 보기 <ChevronDown size={16} /></summary><iframe title={`${week}주차 교육 원문 PDF`} src={source.src} /></details></section>;
}
...
function WeekOne() {
  return <>
    ...
    <section className="paper-callout"><div className="callout-seal"><img src="https://files.manuscdn.com/user_upload_by_module/session_file/91985099/aGkFWUtKZJEVhuMo.png" alt="인도자 교육 심볼" /></div><div><SectionLabel>THE FIRST QUESTION</SectionLabel><h2>오늘 당신의 시선은<br /><span>어디에 고정되어 있습니까?</span></h2><p>인도자의 시선이 예수 그리스도께 고정될 때, 회중의 시선도 함께 방향을 찾습니다.</p></div><ArrowRight className="callout-arrow" /></section>
    <section className="section-block bridge-collapse"><div className="section-intro"><SectionLabel>WHEN THE BRIDGE SHINES</SectionLabel><h2>다리가 스스로 빛나려 할 때,<br /><em>하나님을 향한 길은 무너집니다.</em></h2><p>다리 위를 건너가는 분은 오직 예수 그리스도여야 합니다. 인도자가 자신을 드러내고 목적지가 되려는 순간, 회중을 그리스도께 잇는 다리는 무너집니다.</p></div><div className="bridge-diagram"><span className="bridge-people">회중</span><span className="bridge-self">찬양 인도자</span><span className="bridge-cross">†<small>그리스도</small></span><div className="bridge-beam"><b>인도자가 주인공이 되면</b><i>BRIDGE COLLAPSE</i></div></div></section>
    <section className="section-block identity-formula"><div className="formula-quote"><SectionLabel>JOHN 03 : 30</SectionLabel><blockquote>“그는 흥하여야 하겠고<br />나는 쇠하여야 하리라.”</blockquote><p>개혁주의 찬양 인도자의 영원한 좌우명, 세례요한의 고백입니다.</p></div><div className="formula-board"><div className="formula-side"><span>나</span><b>↓</b><small>작아짐 · 사라짐</small></div><div className="formula-arrow">→</div><div className="formula-side large"><span>예수 그리스도</span><b>↑</b><small>그리스도의 말씀이 풍성이 거하게<br />골로새서 3장 16절</small></div></div><p className="formula-note">내 안의 우상 공장을 멈추는 유일한 방법은 ‘나의 쇠함’입니다. 내가 철저히 작아지고 사라질 때, 그 빈 공간에 그리스도의 말씀이 채워집니다.</p></section>
    <section className="assignment-panel"><div><SectionLabel light>COMMITMENT ASSIGNMENT</SectionLabel><h2>오직 주님만<br /><em>드러나게 하소서</em></h2><p>‘나의 영광 꺾기 인도자 헌신기 선언문’을 A4 1페이지 내외로 작성해 다음 주까지 제출합니다.</p></div><ol><li><span>01</span>최근 회중 앞에서 ‘나’가 너무 드러났던 사례 또는 자기반성 고백</li><li><span>02</span>요한복음 3장 30절에 비추어, 내가 더 작아져야 할 구체적 영역</li><li><span>03</span>오직 예수님만 예배의 주인공이 되게 하겠다는 나의 구체적 다짐</li></ol><div className="assignment-ending">내가 무대에서 완전히 사라질 때,<br />비로소 진짜 예배가 시작됩니다.</div></section>
    <SourcePDF week={1} />
  </>;
}
...
function WeekTwo() {
  ...
  return <>
    ...
    <section className="scripture-closure"><SectionLabel light>THE SOURCE OF TRUE COMFORT</SectionLabel><p className="closure-lead">참된 위로의 유일한 근원은<br />오직 하나님의 말씀입니다.</p><blockquote>“내가 산을 향하여 눈을 들리라<br />나의 도움이 어디서 올까<br />나의 도움은 천지를 지으신<br />여호와에게서로다”</blockquote><small>시편 121편 1–2절 · 개역개정</small><p>흔들리는 감정이나 사람이 만들어낸 발라드가 아니라, 영원불변하신 창조주 하나님 중심(Theocentrism)으로 돌아갈 때 비로소 진정한 위로와 평안을 얻을 수 있습니다.</p></section>
    <section className="section-block"><div className="section-heading"><div><SectionLabel>PRACTICE / TARGET–DIAGNOSTIC–PRESCRIPTION</SectionLabel><h2>실천 과제: 당신의<br /><em>분별력을 증명하십시오.</em></h2></div><div className="heading-note">A4 1 PAGE<br /><span>BEFORE NEXT WEEK</span></div></div><div className="practice-grid"><article><span>01 / TARGET</span><h3>대상을 찾으세요</h3><p>인본주의적 위로에 치중된 CCM 1곡을 선정하고 가사를 추출합니다.</p></article><article><span>02 / DIAGNOSTIC</span><h3>오류를 해부하세요</h3><p>칼빈의 예배 규범적 원리를 적용해 ‘대상 상실의 오류’ 또는 ‘복음 소거 현상’을 분석합니다.</p></article><article><span>03 / PRESCRIPTION</span><h3>말씀의 대안을 쓰세요</h3><p>시편 121편 등 개역개정 본문을 인용하여 하나님 중심적(Theocentrism) 대안을 제시합니다.</p></article></div><div className="practice-note">다음 주차 모임 전까지 제출 완료. <b>우리가 부르는 가사는 곧 우리의 교리입니다.</b></div></section>
    <SourcePDF week={2} />
  </>;
}
...
function WeekThree() {
  ...
  return <>
    ...
    <section className="section-block"><div className="section-heading"><div><SectionLabel>BUILDING THE TRANSITION BRIDGE</SectionLabel><h2>은혜의 다리 놓기:<br /><em>전환을 보호하십시오.</em></h2><p>브릿지에서 최고조에 달한 영적 감정선을 갑자기 끊으면 회중의 은혜가 단절됩니다. 음악적 연결은 듣기 좋은 기술을 넘어, 성도들이 하나님의 임재에서 헌신으로 나아가도록 흐름을 지키는 거룩한 장치입니다.</p></div></div><div className="transition-grid"><BlueprintCard icon={Route} label="TRANSITION 01" title="키 변조와 허밍" body="인도자의 허밍이나 짧은 기도 멘트로 새 키와 인트로에 부드럽게 착륙합니다." /><BlueprintCard icon={Layers3} label="TRANSITION 02" title="신디사이저 패드" body="은은하고 지속적인 패드 사운드로 침묵의 빈틈을 메우고 영적 긴장감을 연결합니다." /><BlueprintCard icon={Gauge} label="TRANSITION 03" title="드럼 킥 펄스" body="심장 박동 같은 일정한 펄스의 리듬으로 흐름이 끊기지 않도록 유지합니다." /></div></section>
    <section className="assignment-panel liturgy-assignment"><div><SectionLabel light>YOUR HOLY BLUEPRINT</SectionLabel><h2>당신만의 거룩한<br /><em>설계도를 완성하십시오.</em></h2><p>구속사적 서사를 담은 고등부 주일 찬양 콘티 4곡을 기획합니다.</p></div><ol><li><span>01</span>부름–참회–구속–헌신의 4단계에 따른 콘티 시트 1장: 곡명, 작곡/작사가, 원곡 키, BPM 포함</li><li><span>02</span>송폼(Verse–Chorus–Bridge) 및 전환 가이드 1장: 키 변조, 악기 편곡, 멘트·허밍 연결 방법을 상세 기술</li></ol><div className="assignment-ending">인도자는 음악 감독이 아니라<br />회중의 영혼을 인도하는 목회자입니다.</div></section>
    <SourcePDF week={3} />
  </>;
}
...
function WeekFour() {
  ...
  return <>
    <section className="dark-week-hero"><div className="dark-hero-copy"><SectionLabel light>THE EDGE BEHIND THE MIC</SectionLabel><h2>마이크 뒤의<br /><em>칼날</em></h2><p>말 한마디로 예배를 살리기도, 사소한 정죄로 공동체를 박살낼 수도 있습니다.</p><div className="dark-hero-meta"><span>WEEK 04</span><span>LANGUAGE STEWARDSHIP</span></div></div><div className="mic-visual"><img src="https://files.manuscdn.com/user_upload_by_module/session_file/91985099/HjNcOnEdDMMZlZZd.jpg" alt="빛을 받는 마이크와 말씀의 상징" /></div></section>
    <section className="section-block language-steward"><div className="section-heading"><div><SectionLabel>STEWARD OF WORDS</SectionLabel><h2>우리는 가수가 아니라<br /><em>언어의 청지기</em>입니다.</h2><p>멘트의 본질은 사생활 나눔이 아니라 성경 말씀의 명확한 선포입니다. 인도자의 말은 회중의 시선을 개인에게 붙들어 두는 소리가 아니라 하나님께 고정하는 통로가 되어야 합니다.</p></div></div><div className="scripture-cards"><article><BookOpen size={18} /><strong>잠언 10장 19절</strong><p>말이 많으면 허물을 면하기 어려우나, 그 입술을 제어하는 자는 지혜가 있습니다.</p></article><article><BookOpen size={18} /><strong>에베소서 4장 29절</strong><p>오직 덕을 세우는 데 소용되는 대로 선한 말을 하여 듣는 자들에게 은혜를 끼치게 하십시오.</p></article><article><BookOpen size={18} /><strong>느헤미야 8장 5–8절</strong><p>에스라는 율법책을 펴고 낭독하며 그 뜻을 해석하여 백성에게 깨닫게 했습니다.</p></article></div><div className="language-contrast"><div><span>인간 중심의 감성 팔이</span><ul><li>개인 사생활 나눔</li><li>인도자 개인에게 시선 집중</li><li>길고 장황한 수다</li></ul></div><ArrowRight size={20} /><div><span>말씀 중심의 선포</span><ul><li>개역개정 성경 구절의 짧고 명확한 선포</li><li>회중의 시선을 삼위일체 하나님께 고정</li><li>하나님을 향한 간결한 찬양의 초청</li></ul></div></div></section>
